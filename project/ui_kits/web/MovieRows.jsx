...
function MovieRow({ title, link, movies, onOpen }) {
  ...
  return (
    <section style={{padding:'40px 64px 0', maxWidth:1440, margin:'0 auto'}}>
      <div style={{
        ...
        <div style={{display:'flex', alignItems:'center', gap:14}}>
          ...
          <div style={{display:'flex', gap:6}}>
            <button disabled={!canPrev} onClick={()=>setStart(Math.max(0,start-1))} style={{
            ...
            <button disabled={!canNext} onClick={()=>setStart(start+1)} style={{
              width:34, height:34, borderRadius:'50%', border:'1px solid rgba(255,255,255,.12)',
              background:'rgba(255,255,255,.04)', color:'#fff',
      ...
      {/* overflow-hidden carousel viewport. We add vertical padding + matching
          negative margin so a hovered card's translateY(-4) + scale(1.03) lift
          isn't clipped at the top/bottom edges. */}
...
function TopTen({ movies, onOpen }) {
  // Card width + number-overlap = 140 + 30 (number bleed) = ~170, plus gap 24.
  // We use a scroll container with wheel-to-scroll + next/prev buttons.
  const scrollerRef = useRef(null);
  const STEP = 380; // ~2 cards per click
  ...
  // Translate vertical wheel to horizontal scroll while hovering the row
  const onWheel = (e) => {
    ...
    <section style={{padding:'40px 0 0', maxWidth:1440, margin:'0 auto'}}>
      <div style={{
        ...
        <h2 style={{
          ...
          <span style={{display:'inline-block', width:5, height:24,
            background:'linear-gradient(180deg,#ffc83a,#ff8a00)', borderRadius:3
          }}/>
          Top 10 Đang Hot
        </h2>
        <div style={{display:'flex', gap:6}}>
          <button onClick={()=>scrollBy(-STEP)} style={{
            width:34, height:34, borderRadius:'50%', border:'1px solid rgba(255,255,255,.12)',
      ...
      <style>{`
        .np-top10-scroll::-webkit-scrollbar { display: none; }
      `}</style>
      <div
        ref={scrollerRef}
